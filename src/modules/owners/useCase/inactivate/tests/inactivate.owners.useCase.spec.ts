...
describe('Inactivate Owners Use Case', () => {
  ...
  it('Should be able inactivate a owner', async () => {
    ...
    const result = await inactivateUseCase.execute(id, defaultHeaders);
  ...
  it('Bad Request Exception - This owner already inactive', async () => {
    expect.hasAssertions();
    ...
    try {
      ...
      expect(error.message).toBe('This owner already inactive');
